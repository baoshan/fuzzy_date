...
namespace ICMD.DB.CLR
{
  public partial class UDFs
  {
    ...
    public static SqlBinary fuzzy_date_binary_from_string(SqlString sql_string)
    {
      // NULL in NULL out.
      ...
      try
      {
        ...
        if (!fuzzy_date_is_valid_binary(sql_binary)) { return SqlBinary.Null; }

        // Ensure the output is the same as input.
        if (input.Replace(".", "") != fuzzy_date_string_from_binary(sql_binary).Value.Replace(" ", "").Replace(".", "").ToLower()) { return SqlBinary.Null; }
